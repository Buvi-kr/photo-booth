...
## 🔜 향후 업데이트 계획 (Roadmap - 4/30 예정)

사용자 편의성 극대화 및 어르신 친화적 UX를 위해 아래 사항들을 개선할 예정입니다.

1.  **UI 간소화 및 터치 최소화:**
    *   '촬영하기' 버튼 및 하단 회색 레이어 제거.
    *   **배경 썸네일 클릭 즉시** 촬영 프로세스가 시작되도록 트리거 통합.
2.  **타이머 및 대기 시간 조정:**
    *   촬영 전 준비 시간을 5초에서 **10초**로 상향 (포즈 잡기 및 시선 처리 여유 확보).
    *   촬영 후 QR 스캔 및 결과 확인 시간을 **60초**로 설정하여 어르신들의 사용 편의성 증대.
3.  **프로세스 자동화:**
    *   촬영 후 무조작 시 60초 뒤 자동으로 초기 화면(배경 선택)으로 복귀.
    *   '재촬영' 클릭 시 배경 선택 단계를 건너뛰고 **즉시 현재 배경으로 10초 카운트다운** 시작.
