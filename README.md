...
### [2026.06.18] 🌐 공유기 탈착 대응 스마트 터널 복구 & 메모리 누수 리팩토링
**주요 성과:** 공유기가 빠지는 등 인터넷이 차단된 특수 상황에서 무차별 재시작으로 인한 Rate Limit(시간당 6회) 소모를 방지하는 스마트 복구 시스템 도입 및 Kiosk 장기 무인 구동 시 메모리 부족(OOM)으로 인한 크래시 위험을 원천 차단하는 리소스크 클린업 리팩토링 완료.

*   **인터넷 완전 단절 감지 및 재시작 보류 (Smart Guard):**
    *   헬스 체크가 3회 연속 실패(총 30초)하더라도 `Application.internetReachability`를 통해 인터넷 상태를 먼저 파악합니다.
    *   인터넷 자체가 끊긴 상황이라면 프로세스 재시작을 일시 보류(Pause)하여 횟수 한도가 조기 소진되는 현상을 방지합니다.
    *   이후 인터넷 복구 감지 시 터널 재시작을 즉시 자동 트리거합니다.
*   **사용자 경험 보호를 위한 결과 화면(Result) 재시작 유예:**
    *   사용자가 결과 화면에서 QR 코드를 스캔 중일 수 있으므로 터널 상태가 비활성화되더라도 터널 재시작을 즉각 유예하여 QR 코드 먹통 현상을 막습니다.
    *   홈 화면(`Standby`/`SelectBG` 등)으로 복귀하는 즉시 대기 중이던 터널 재시작을 수행하여 새로운 클린 URL을 발급받습니다.
*   **10초 주기 터널 루프백 헬스 체크 (`/health` 엔드포인트 신설):**
    *   내장 HTTP 서버에 메모리 기반 `/health` 엔드포인트를 구현하고, 10초 주기로 자가 진단(Self-Ping)을 실행하여 통신 오류와 먹통 상태를 신속하게 감지합니다.
    *   헬스 체크 도중 터널 자동 재연결이 감지되면 프로세스를 불필요하게 죽이지 않고 `isServerReady`만 자동으로 복구하여 기존 URL을 최대한 보호합니다.
*   **메모리 누수 방지 리팩토링 (Kiosk OOM 방지):**
    *   **QR 코드 텍스처 누수 해결**: [QRServerManager.cs](file:///c:/Users/Buvi/3gallely_photo/Assets/Scripts/Core/QRServerManager.cs)에서 QR 코드를 재생성하거나 삭제할 때 기존 이미지 텍스처를 명시적으로 `Destroy()`하여 누적 OOM 요소를 제거했습니다.
    *   **썸네일 동적 Sprite 누수 해결**: [OverlayBGManager.cs](file:///c:/Users/Buvi/3gallely_photo/Assets/Scripts/Core/OverlayBGManager.cs)에서 버튼 썸네일로 실시간 구워 사용하던 Sprite들을 캐시 배열을 통해 관리하고 갱신/종료 시 명시적으로 `Destroy()` 처리하여 리소스를 정리했습니다.
