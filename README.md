...
*   **캡처 트랜스폼 및 크롭 완벽 동기화:**
    *   **Shader-based Transform (Perfect Sync):** `Graphics.Blit`의 속성 초기화 문제를 방지하기 위해 커스텀 `_CaptureST` 변수를 도입하고, **버텍스 셰이더 최적화 연산(SRT 순서)**을 통해 줌/이동/회전 상태에서도 미리보기와 100% 일치하는 결과물 생성.
    *   **셰이더 기반 크롭(Crop) 및 페이딩:** UI 마스크 대신 셰이더 알파 마스킹을 사용하여 배경/프레임을 보존하면서 인물만 정교하게 크롭. 원본 좌표계 분리를 통해 복잡한 역계산 없이 UI 정합성 확보.
*   **시스템 안정성 강화:**
